...
public class XMLStreamReaderImpl implements XMLStreamReader {
    ...
    for (int eventType = next();
      ...
      switch (eventType) {
        ...
        case ENTITY_REFERENCE:
          sb.append(new String (_cBuf, 0, _cBufLength));
          break;
  ...
  {
    char []buf = new char[_cBuf.length];
    System.arraycopy(_cBuf, 0, buf, 0, _cBuf.length);
    return buf;
  }
      ...
      switch (ch) {
        ...
        case '"': case '\'':
          if (ch == end)
            ...
            valueBuffer[valueIndex++] = (byte) ch;
          break;
    ...
    throws XMLStreamException
  {
    try {
      if (_inputLength <= _inputOffset) {
        ...
        _inputBuf[_inputLength++] = (byte) ch;
    ...
    throws XMLStreamException
  {
    try {
      if (_is != null) {
        ...
        _inputOffset = 0;
        char []buf = new char[_inputBuf.length];
        _inputLength = _reader.read(buf, 0, _inputBuf.length);
        System.arraycopy(buf, 0, _inputBuf, 0, _inputBuf.length);
