...
import com.clevercloud.xpath.functions.BaseURI;
import com.clevercloud.xpath.functions.ResolveURI;
import com.clevercloud.xpath.functions.Trace;
...
class XPathParser {
   ...
   Expr parseFunction(AbstractPattern parent,
      ...
      switch (code) {
         ...
         case Expr.LOCAL_PART:
         case Expr.NAMESPACE:
         case Expr.QNAME:
         case Expr.GENERATE_ID:
            if (args.size() == 0)
               args.add(NodeSetExpr.create(new FromContext()));
            return new StringExpr(code, args);
