 ...
 * PHP error handling.
 */
public class ErrorModule extends AbstractBiancaModule {
...
//            "UNKNOWN:" + className.charAt(i + 1) + " " + className);
   ...
   private static String unmangleClass(String className) {
      int p = className.lastIndexOf("$bianca_");
      int q = className.lastIndexOf("$");

      if (p > 0 && p < q) {
         className = className.substring(p + "$bianca_".length(), q);
      }
