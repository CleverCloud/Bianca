...
public class ReflectionClass
   ...
   public String _name;
   private BiancaClass _cls;

   protected ReflectionClass(BiancaClass cls) {
      _cls = cls;
      _name = cls.getName();
   }
   ...
   protected BiancaClass getBiancaClass() {
      return _cls;
   }
   ...
   public static ReflectionClass __construct(Env env, Value obj) {
      ...
      if (obj.isObject()) {
         cls = ((ObjectValue) obj.toValue()).getBiancaClass();
      } else {
   ...
   private void findInterfaces(Env env, ArrayValue array, ClassDef def) {
      ...
      if (def.isInterface()) {
         ...
         for (int i = 0; i < defList.length; i++) {
            BiancaClass cls = env.findClass(defList[i]);
   ...
   public boolean isInstance(ObjectValue obj) {
      return obj.getBiancaClass().getName().equals(_name);
   }
   ...
   public ReflectionClass getParentClass() {
      BiancaClass parent = _cls.getParent();
   ...
   private void getStaticFields(Env env, ArrayValue array, BiancaClass cls) {
      if (cls == null) {
         return;
      }
