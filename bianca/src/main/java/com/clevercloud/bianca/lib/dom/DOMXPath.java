...
public class DOMXPath {
   ...
   private NodeList query(Env env, String pattern, Node node) {
      ...
      try {
         Expr expr = com.clevercloud.xpath.XPath.parseExpr(pattern);
   ...
   try {
   if (_context == null) {
   Bianca bianca = env.getBianca();

   ExpressionCache cache
   = (ExpressionCache) bianca.getSpecial("clevercloud.domxpath.cache");

   if (cache == null) {
   cache = new ExpressionCache();
   bianca.setSpecial("clevercloud.domxpath.cache", cache);
   }
   ...
   XPath xpath = (XPath) env.getSpecialValue("clevercloud.domxpath.xpath");

   if (xpath == null) {
   XPathFactory factory = XPathFactory.newInstance();
   xpath = factory.newXPath();
   env.setSpecialValue("clevercloud.domxpath.xpath", xpath);
   }
