 ...
 * PHP options
 */
public class OptionsModule extends AbstractBiancaModule {
   ...
   private static final String phpVersion = (new BiancaContext()).getPhpVersion();
   ...
   public static Value q_assert(Env env, String code) {
      try {
         ...
         BiancaProgram program = bianca.parseCode(code);
   ...
   public static Value get_include_path(Env env) {
      return BiancaContext.INI_INCLUDE_PATH.getAsStringValue(env);
   }
   ...
   private static Value getAllDirectives(Env env, String prefix) {
      ...
      IniDefinitions iniDefinitions = env.getBianca().getIniDefinitions();
      ...
      for (String name : names) {
         if (name.startsWith(prefix)) {
            ...
            if (!iniDefinition.isRuntimeDefinition()) {
               ...
               inner.put(global, iniDefinition.getAsStringValue(env.getBianca()));
   ...
   public static String php_sapi_name(Env env) {
      return env.getBianca().getSapiName();
   }
   ...
   private static Value escape(Env env, Value value) {
      if (value.isArray()) {
         ...
         ObjectValue result = new ObjectExtValue(obj.getBiancaClass());
