...
import com.clevercloud.bianca.expr.LiteralExpr;
import com.clevercloud.bianca.function.AbstractFunction;
import com.clevercloud.bianca.marshal.JavaMarshal;
...
public class JavaClassDef extends ClassDef {
   ...
   private BiancaClass _biancaClass;
   ...
   public Value getField(Env env, Value qThis, StringValue name) {
      ...
      if (get != null) {
         try {
            return get.callMethod(env, getBiancaClass(), qThis);
         } catch (Exception e) {
            log.log(Level.FINE, L.l(e.getMessage()), e);
      ...
      AbstractFunction phpGet = qThis.getBiancaClass().getFieldGet();

      if (phpGet != null) {
         return phpGet.callMethod(env, getBiancaClass(), qThis, name);
      }

      if (__fieldGet != null) {
         try {
            return __fieldGet.callMethod(env, getBiancaClass(), qThis, name);
         } catch (Exception e) {
            log.log(Level.FINE, L.l(e.getMessage()), e);
   ...
   public Value putField(Env env,
      ...
      if (setter != null) {
         try {
            return setter.callMethod(env, getBiancaClass(), qThis, value);
         } catch (Exception e) {
            log.log(Level.FINE, L.l(e.getMessage()), e);
      ...
      if (!qThis.isFieldInit()) {
         AbstractFunction phpSet = qThis.getBiancaClass().getFieldSet();

         if (phpSet != null) {
            qThis.setFieldInit(true);

            try {
               return phpSet.callMethod(env, getBiancaClass(), qThis, name, value);
    ...
    */
   @Override
   public ObjectValue newInstance(Env env, BiancaClass qClass) {
      // return newInstance();
      return null;
   }
   ...
   public Value callNew(Env env, Value[] args) {
      if (_cons != null) {
         if (__construct != null) {
            ...
            __construct.callMethod(env, __construct.getBiancaClass(), value, args);
    ...
    */
   public BiancaClass getBiancaClass() {
      if (_biancaClass == null) {
         init();

         _biancaClass = new BiancaClass(_moduleContext, this, null);
      }
   ...
   private void introspectAnnotations(Class type) {
      try {
         ...
         throw new BiancaModuleException(e.getCause());
      } catch (Exception e) {
   ...
   private void introspectFields(ModuleContext moduleContext, Class type) {
      ...
      for (Method method : methods) {
         ...
         if (length > 3) {
            if (methodName.startsWith("get")) {
               StringValue biancaName = javaToBiancaConvert(methodName.substring(3, length));

               AbstractJavaMethod existingGetter = _getMap.get(biancaName);
               ...
               _getMap.put(biancaName, newGetter);
            } else if (methodName.startsWith("is")) {
               StringValue biancaName = javaToBiancaConvert(methodName.substring(2, length));
               ...
               AbstractJavaMethod existingSetter = _setMap.get(biancaName);
               ...
               _setMap.put(biancaName, newSetter);
      ...
      */
      ...
      // introspectFields(bianca, type.getSuperclass());
   }
    ...
    * @return (foo, URL)
    */
   private StringValue javaToBiancaConvert(String s) {
      if (s.length() == 1) {
   ...
   private void introspectConstants(Class type) {
      ...
      for (Field field : fields) {
         ...
         try {
            ...
            Value value = BiancaContext.objectToValue(obj);
