...
import com.clevercloud.bianca.program.UnsetFunction;
import com.clevercloud.util.Crc64;
...
public final class DefinitionState {
   ...
   private static final LruCache<ClassKey, SoftReference<BiancaClass>> _classCache = new LruCache<ClassKey, SoftReference<BiancaClass>>(4096);
   ...
   public DefinitionState(BiancaContext bianca) {
      _bianca = bianca;
   ...
   public ArrayValue getDefinedFunctions() {
      ...
      ArrayValue internal = _bianca.getDefinedFunctions();
   ...
   public Value getDeclaredClasses(Env env) {
      ...
      for (String name : _bianca.getClassMap().keySet()) {
         if (!names.contains(name)) {
            names.add(name);
         }
      }
