...
import com.clevercloud.bianca.lib.OptionsModule;
import com.clevercloud.bianca.lib.VariableModule;
import com.clevercloud.bianca.lib.file.FileModule;
import com.clevercloud.bianca.lib.file.PhpStderr;
import com.clevercloud.bianca.lib.file.PhpStdin;
import com.clevercloud.bianca.lib.file.PhpStdout;
import com.clevercloud.bianca.lib.regexp.RegexpState;
...
public class Env {
   ...
   private static final FreeList<BiancaClass[]> _freeClassList = new FreeList<BiancaClass[]>(256);
   ...
   protected final BiancaContext _bianca;
   private BiancaPage _page;
   ...
   private HashMap<Path, BiancaPage> _includeMap = new HashMap<Path, BiancaPage>();
   ...
   private BiancaClass _callingClass;
   ...
   // hold special Bianca php import statements
   ...
   public Env(BiancaContext bianca,
              BiancaPage page,
              WriteStream out,
      ...
      _isStrict = bianca.isStrict();

      _isAllowUrlInclude = bianca.isAllowUrlInclude();
      _isAllowUrlFopen = bianca.isAllowUrlFopen();

      _variablesOrder = _bianca.getIniValue("variables_order").toStringValue(this);

      StringValue querySeparators = _bianca.getIniValue("arg_separator.input").toStringValue(this);
      ...
      // _defState = new DefinitionState(bianca);
      ...
      ClassDef[] defClasses = bianca.getClassDefMap();
      ...
      if (_qClass == null || _qClass.length < defClasses.length) {
         _qClass = new BiancaClass[defClasses.length];
      } else {
         // list should have been zeroed on call to free
      }

      Value[] defConst = bianca.getConstantMap();
      ...
      _internalAutoload = new InternalAutoloadCallback("com/clevercloud/bianca/php/");
   ...
   public Env(BiancaContext bianca) {
      this(bianca, null, null, null, null);
   }
   ...
   protected final AbstractFunction[] getDefaultFunctionMap() {
      return getBianca().getFunctionMap();
   }

   /**
    * Initialize the page, loading any functions and classes
    */
   protected final BiancaPage pageInit(BiancaPage page) {
   ...
   public String getScriptEncoding() {
      ...
      if (encoding.length() == 0) {
         ...
         if (encoding.length() == 0) {
            return getBianca().getScriptEncoding();
         }
      }
   ...
   public String getOutputEncoding() {
      String encoding = BiancaContext.INI_UNICODE_OUTPUT_ENCODING.getAsString(this);

      if (encoding == null) {
         encoding = BiancaContext.INI_UNICODE_FALLBACK_ENCODING.getAsString(this);
      }
   ...
   public ServletContext getServletContext() {
      return _bianca.getServletContext();
   }
   ...
   public void start() {
      ...
      _startTime = _bianca.getCurrentTime();
      ...
      HashSet<ModuleStartupListener> listeners = _bianca.getModuleStartupListeners();

      for (ModuleStartupListener listener : listeners) {
         listener.startup(this);
      }

      _bianca.startEnv(this);
   }
   ...
   public long getCurrentTime() {
      return getBianca().getCurrentTime();
   }
   ...
   public ModuleContext getModuleContext() {
      return _bianca.getModuleContext();
   }
   ...
   public DataSource getDatabase() {
      return _bianca.getDatabase();
   }

   protected final DataSource findDatabase(String driver, String url)
      throws Exception {
      return _bianca.findDatabase(driver, url);
   }
   ...
   public ConnectionEntry getConnection(String driver, String url,
      ...
      DataSource database = _bianca.getDatabase();
   ...
   public String generateSessionId() {
      String sessionId =
         _bianca.getBiancaSessionManager().createSessionId(this);
   ...
   public SessionArrayValue createSession(String sessionId, boolean create) {
      long now = _bianca.getCurrentTime();
      ...
      SessionArrayValue session = _bianca.loadSession(this, sessionId);
   ...
   public void destroySession(String sessionId) {
      ...
      if (callback != null) {
         ...
         _bianca.destroySession(sessionId);
      }
   ...
   public Logger getLogger() {
      if (_logger == null) {
         _logger = Logger.getLogger("bianca.bianca");
      }
   ...
   public Value getConfigVar(String name) {
      return getIniDefinition(name).getValue(_bianca);
   }
   ...
   private IniDefinition getIniDefinition(String name) {
      return _bianca.getIniDefinitions().get(name);
   }
   ...
   public final StringValue createStaticName() {
      return _bianca.createStaticName();
   }
   ...
   // php/324a
   // php/324b
   BiancaClass callingClass = getCallingClass(qThis);
   ...
   private EnvVar getSuperGlobalRef(StringValue name,
      ...
      switch (specialVarId) {
         ...
         case HTTP_POST_VARS:
            if (!BiancaContext.INI_REGISTER_LONG_ARRAYS.getAsBoolean(this)) {
               return null;
            } else {
         ...
         case HTTP_RAW_POST_DATA: {
            if (!BiancaContext.INI_ALWAYS_POPULATE_RAW_POST_DATA.getAsBoolean(this)) {
         ...
         case _SERVER: {
            ...
            if (_variablesOrder.indexOf('S') >= 0) {
               ...
               if (_bianca.getIniBoolean("register_argc_argv")
                  && query != null) {
   ...
   protected EnvVar getGlobalSpecialRef(StringValue name) {
      if (BiancaContext.isSuperGlobal(name)) {
         return _globalMap.get(name);
      } else {
         return null;
      }
    ...
    */
   public BiancaClass getCallingClass() {
      return _callingClass;
   }

   public Value getCallingClassName() {
      BiancaClass qClass = _callingClass;
    ...
    * Returns the calling class.
    */
   public BiancaClass getCallingClass(Value qThis) {
    ...
    */
   public BiancaClass setCallingClass(BiancaClass cls) {
      BiancaClass oldCallingClass = _callingClass;
   ...
   private Value getConstantImpl(String name) {
      int id = _bianca.getConstantId(name);
      ...
      id = _bianca.getConstantLowerId(name);
      ...
      /*
      id = _bianca.getConstantLower(id);
      ...
      /*
      value = _bianca.getConstant(name);
      if (value != null)
      return value;
   ...
   public Value getConstant(int id) {
      if (_const.length <= id) {
         return _bianca.getConstantName(id);
      }
      ...
      int lowerId = _bianca.getConstantLower(id);
      ...
      Value nameValue = _bianca.getConstantName(id);
   ...
   public final Value addConstant(String name,
      ...
      if (isCaseInsensitive) {
         ...
         id = _bianca.addLowerConstantId(newname);
      } else {
         id = _bianca.getConstantId(name);
      }
   ...
   public Value addConstant(StringValue name,
      ...
      if (isCaseInsensitive) {
         id = _bianca.addLowerConstantId(name);
      } else {
         id = _bianca.getConstantId(name);
      }
   ...
   public ArrayValue getDefinedConstants() {
      ...
      for (int i = 0; i < _const.length; i++) {
         if (_const[i] != null) {
            result.append(_bianca.getConstantName(i), _const[i]);
         }
      }
   ...
   public boolean isExtensionLoaded(String name) {
      return getBianca().isExtensionLoaded(name);
   }
   ...
   public HashSet<String> getLoadedExtensions() {
      return getBianca().getLoadedExtensions();
   }
   ...
   public Value getExtensionFuncs(String name) {
      return getBianca().getExtensionFuncs(name);
   }
   ...
   public ArrayValue getDefinedFunctions() {
      ...
      AbstractFunction[] systemFuns = _bianca.getFunctionMap();
   ...
   public int findFunctionId(String name) {
      return _bianca.findFunctionId(name);
   }
   ...
   public AbstractFunction findFunction(String name) {
      int id = _bianca.findFunctionId(name);
      ...
      /*
      AbstractFunction fun = _bianca.findFunctionImpl(name);
      ...
      id = _bianca.findFunctionId(name);
      ...
      fun = _bianca.findLowerFunctionImpl(name);
   ...
   {
   int id = _bianca.getFunctionId(name);
   ...
   AbstractFunction []defFuns = _bianca.getFunctionMap();
   ...
   {
   return _bianca.getFunctionIdCount();
   }
    */
   ...
   public Value addFunction(String name, AbstractFunction fun) {
      AbstractFunction staticFun = _bianca.findLowerFunctionImpl(name.toLowerCase());
   ...
   public AbstractFunction createAnonymousFunction(String args, String code)
      ...
      AbstractFunction fun = getBianca().parseFunction(name, args, code);
   ...
   public AbstractFunction findMethod(String className, String methodName) {
      BiancaClass cl = findClass(className);
   ...
   public Value evalCode(String code)
      ...
      BiancaProgram program = bianca.parseEvalExpr(code);
   ...
   public void execute()
      throws IOException {
      StringValue prepend = _bianca.getIniValue("auto_prepend_file").toStringValue(this);
      ...
      if (prepend.length() > 0) {
         ...
         if (prependPath == null) {
            ...
            BiancaPage prependPage = _bianca.parse(prependPath);
      ...
      StringValue append = _bianca.getIniValue("auto_append_file").toStringValue(this);
    ...
    */
   private void uncaughtExceptionError(BiancaLanguageException e) {
    ...
    */
   protected Value executePage(BiancaPage page) {
    ...
    * Executes the given page
    */
   protected Value executePageTop(BiancaPage page) {
      if (page.getCompiledPage() != null) {
         return page.getCompiledPage().execute(this);
      } else {
         return page.execute(this);
   ...
   public void restoreState(SaveState saveState) {
      ...
      BiancaClass[] qClass = saveState.getBiancaClassList();
      ...
      Map<Path, BiancaPage> includeMap = saveState.getIncludeMap();
   ...
   public ObjectValue createObject() {
      try {
         return (ObjectValue) _bianca.getStdClass().createObject(this);
      } catch (Exception e) {
         throw new BiancaModuleException(e);
      }
   }
   ...
   public ObjectValue createIncompleteObject(String name) {
      try {
         ObjectValue obj = (ObjectValue) _bianca.getStdClass().createObject(this);
   ...
   public Value createException(String exceptionClass, String message) {
      BiancaClass cls = getClass(exceptionClass);
   ...
   public Value createException(Throwable e) {
      BiancaClass cls = findClass("Exception");
   ...
   public JavaClassDef getJavaClassDefinition(Class type) {
      JavaClassDef def = _bianca.getJavaClassDefinition(type, type.getName());
   ...
   private JavaClassDef getJavaClassDefinition(String className,
      ...
      try {
         def = _bianca.getJavaClassDefinition(className);
   ...
   public BiancaClass findClass(int id,
      ...
      BiancaClass cl = createClassFromCache(id, useAutoload, useImport);
      ...
      if (cl != null) {
         ...
         String name = _bianca.getClassName(id);

         BiancaClass qcl = findClassExt(name, useAutoload, useImport);
         ...
         if (!_autoloadClasses.contains(name)) {
            try {
               ...
               for (int i = 0; i < size; i++) {
                  ...
                  // php/0977
                  BiancaClass cls = findClass(name, false, useImport);
         ...
         if (importPhpClass(name)) {
            ...
            try {
               ...
               if (javaClassDef != null) {
                  BiancaClass cls = createBiancaClass(id, javaClassDef, null);
    ...
    */
   public BiancaClass getClass(int classId) {
      if (_qClass.length <= classId) {
         BiancaClass[] oldClassList = _qClass;

         _qClass = new BiancaClass[classId + 256];

         System.arraycopy(oldClassList, 0, _qClass, 0, oldClassList.length);
      }

      BiancaClass qClass = _qClass[classId];
      ...
      if (def == null) {
         BiancaClass cl = findClass(classId, true, true);

         if (cl != null) {
            return cl;
         } else {
            error(L.l("'{0}' is an unknown class.",
               _bianca.getClassName(classId)));


            throw new BiancaException(L.l("'{0}' is an unknown class.",
               _bianca.getClassName(classId)));
         }
      }
      ...
      if (def.getParentName() != null) {
         parentId = _bianca.getClassId(def.getParentName());
      }
   ...
   public void addClass(ClassDef def, int classId, int parentId) {
      ...
      // php/0cn2 - make sure interfaces have a BiancaClass
      /* XXX: temp, needs to be argument
     for (String iface : def.getInterfaces()) {
     BiancaClass cl = findClass(iface);
     }
      */

      BiancaClass parentClass = null;

      if (parentId >= 0) {
         parentClass = getClass(parentId);
      }

      BiancaClass qClass = _bianca.getCachedClass(classId);

      if (qClass == null
         || qClass.isModified()
         || qClass.getClassDef() != def
         || qClass.getParent() != parentClass) {
         qClass = createBiancaClass(classId, def, parentClass);

         _bianca.setCachedClass(classId, qClass);
      }
   ...
   private BiancaClass createClassFromCache(int id,
      ...
      if (id < _classDef.length && _classDef[id] != null) {
         ...
         if (parentName == null || parent != null) {
            return createBiancaClass(id, classDef, parent);
         } else {
            return null; // php/
         }
      }

      ClassDef staticClass = _bianca.getClassDef(id);

      if (staticClass != null) {
         return createBiancaClass(id, staticClass, null); // TODO: cache
      } else {
         return null;
      }
   }
    ...
    * @return the found class or null if no class found.
    */
   public BiancaClass findAbstractClass(String name) {
      BiancaClass cl = findClass(name, true, true);
      ...
      /*
      // return _bianca.findJavaClassWrapper(name);
    ...
    * @throws BiancaRuntimeException if the class is not found
    */
   public BiancaClass getClass(String name) {
      BiancaClass cl = findClass(name);
   ...
   BiancaClass createJavaBiancaClass(JavaClassDef def) {
      int id = getBianca().getClassId(def.getName());

      if (_qClass.length <= id) {
         BiancaClass[] oldClassList = _qClass;

         _qClass = new BiancaClass[id + 256];

         System.arraycopy(oldClassList, 0, _qClass, 0, oldClassList.length);
      }

      if (_qClass[id] == null) {
         _qClass[id] = def.getBiancaClass();
      }
   ...
   BiancaClass createBiancaClass(int id,
      ...
      if (qClass == null
         ...
         qClass = new BiancaClass(getModuleContext(), def, parent);
         _qClass[id] = qClass;

         qClass.validate(this);
         _bianca.setCachedClass(id, qClass);
      }
   ...
   public AbstractFunction findFunction(String className, String methodName) {
      ...
      if (cl == null) {
         throw new BiancaRuntimeException(L.l("'{0}' is an unknown class",
            className));
      }
   ...
   BiancaClass cls = findClass(clsName);
   ...
   BiancaClass cl = findClass(obj.toString());
   ...
   public Value include(Path scriptPwd, StringValue include,
      ...
      try {
         ...
         BiancaPage page = _includeMap.get(path);

         if (page != null && isOnce) {
            return BooleanValue.TRUE;
         } else if (page == null || page.isModified()) {
            page = _bianca.parse(path);
   ...
   void executePage(Path path) {
      ...
      try {
         BiancaPage page = _bianca.parse(path);
   ...
   private Path lookupInclude(StringValue include, Path pwd, Path scriptPwd) {
      ...
      Path path = _bianca.getIncludeCache(include, includePath, pwd, scriptPwd);
      ...
      if (path == null) {
         ...
         if (path != null) {
            _bianca.putIncludeCache(include, includePath, pwd, scriptPwd, path);
         }
      }
   ...
   private String getDefaultIncludePath() {
      ...
      if (_includePathIniCount != _iniCount) {
         includePath = BiancaContext.INI_INCLUDE_PATH.getAsString(this);
         _includePath = null;
   ...
   private boolean includeExists(Path path) {
      if (path.canRead() && !path.isDirectory()) {
         return true;
      } else if (!getBianca().isRequireSource()) {
         return getBianca().includeExists(path);
      } else {
         return false;
      }
   }
   ...
   public String setIncludePath(String path) {
      String prevIncludePath = BiancaContext.INI_INCLUDE_PATH.getAsString(this);

      if (_defaultIncludePath == null) {
         _defaultIncludePath = prevIncludePath;
      }

      BiancaContext.INI_INCLUDE_PATH.set(this, path);
   ...
   public void restoreIncludePath() {
      BiancaContext.INI_INCLUDE_PATH.set(this, _defaultIncludePath);
   }
   ...
   public Value exit(Value msg) {
      ...
      throw new BiancaExitException(msg.toString());
   }
   ...
   public Value die(String msg) {
      ...
      throw new BiancaDieException(msg);
   }
    ...
    * A fatal runtime error.
    */
   public BiancaRuntimeException createErrorException(String msg)
   ...
   public BiancaRuntimeException createErrorException(Location location,
      ...
      return new BiancaRuntimeException(fullMsg);
   }
   ...
   public Value warning(String msg) {
      ...
      if ((getErrorMask() & mask) != 0) {
         if (log.isLoggable(Level.FINER)) {
            BiancaException e = new BiancaException(msg);
   ...
   public Value error(int code, Location location, String loc, String msg) {
      ...
      if (log.isLoggable(Level.FINEST)) {
         BiancaException e = new BiancaException(loc + msg);
      ...
      if ((mask & (E_ERROR | E_CORE_ERROR | E_COMPILE_ERROR | E_USER_ERROR)) != 0) {
         ...
         BiancaErrorException exn;

         if (!"".equals(locPrefix)) {
            /*
            throw new BiancaLineExitException(getLocation() +
            getCodeName(mask) +
            msg);
             */
            exn = new BiancaErrorException(locPrefix
               + getCodeName(mask)
               + msg);
         } else {
            exn = new BiancaErrorException(msg);
         }
   ...
   public static String[] getSourceLine(Path path, int sourceLine, int length) {
      if (path == null) {
         ...
         // for BiancaScriptEngine.eval() where only a Reader is passed in
         // TODO: not too pretty
         return null;
      }
   ...
   public long getMicroTime() {
      long nanoTime = _bianca.getExactTimeNanoseconds();

      if (_firstMicroTime <= 0) {
         _firstNanoTime = nanoTime;

         _firstMicroTime = _bianca.getExactTime() * 1000
   ...
   private void cleanup() {
      ...
      if (fun != null) {
          ...
          * the (BiancaContext)-Constructor and therefor every
         ...
         for (BiancaPage page : _includeMap.values()) {
   ...
   public void sessionWriteClose() {
      ...
      if (session != null) {
         ...
         if (callback != null) {
            ...
            _bianca.saveSession(this, session);
   ...
   static class ClassKey {
      ...
      private final WeakReference<BiancaClass> _parentRef;
      private final int _hash;

      ClassKey(ClassDef def, BiancaClass parent) {
         _defRef = new WeakReference<ClassDef>(def);

         if (parent != null) {
            _parentRef = new WeakReference<BiancaClass>(parent);
         } else {
            _parentRef = null;
         }
      ...
      public boolean equals(Object o) {
         ...
         BiancaClass aParent = _parentRef.get();
         BiancaClass bParent = key._parentRef.get();
