...
abstract public class Statement {
   ...
   final protected <E extends Throwable> void rethrow(Throwable t, Class<E> cl)
      ...
      if (!(rootCause instanceof BiancaExecutionException)) {
         String rootCauseName = rootCause.getClass().getName();
         String rootCauseMessage = rootCause.getMessage();

         StringBuilder biancaExMessage = new StringBuilder();

         biancaExMessage.append(rootCauseName);

         if (rootCauseMessage != null && rootCauseMessage.length() > 0) {
            biancaExMessage.append(" ");
            biancaExMessage.append(rootCauseMessage);
         }

         BiancaExecutionException biancaEx = new BiancaExecutionException(biancaExMessage.toString());

         StackTraceElement[] biancaExStackTrace = biancaEx.getStackTrace();
         StackTraceElement[] rootCauseStackTrace = rootCause.getStackTrace();

         int biancaExIndex = biancaExStackTrace.length - 1;
         int rootCauseIndex = rootCauseStackTrace.length - 1;

         while (rootCauseIndex >= 0 && biancaExIndex >= 0) {
            StackTraceElement rootCauseElement = rootCauseStackTrace[rootCauseIndex];
            StackTraceElement biancaExElement = biancaExStackTrace[biancaExIndex];

            if (!biancaExElement.equals(rootCauseElement)) {
               break;
            }
         ...
         biancaEx.setStackTrace(trimmedElements);
         try {
            rootCause.initCause(biancaEx);
            rootCause = biancaEx;
